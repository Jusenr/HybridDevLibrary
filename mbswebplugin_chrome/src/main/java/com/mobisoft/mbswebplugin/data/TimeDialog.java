...
//	private boolean issetdata = true;
	...
	public TimeDialog(Context context, int currentHour, int currentMinute) {
		super(context, R.layout.dialog_picker_center);
		this.context = context;
...
//		VDialogPicker.addView(wvDate);
		...
		if (!issetdata&&!issethour) {
			initTime();
		}else if (!issetdata&&issethour) {
...
//					ToastUtil.showLongToast(context, "不能选择当前时间之前的时间");
...
//					ToastUtil.showLongToast(context, "不能选择当前时间之前的时间");
	 ...
	 * @param currMinute
	 */
	public void setTime(int currHour ,int currMinute){
		Calendar c = Calendar.getInstance();
		setTime(c.get(Calendar.YEAR),
				c.get(Calendar.MONTH) + 1,
				c.get(Calendar.DAY_OF_MONTH),
	...
	public void onClick(View v) {
		...
		if (v == btnSure) {
			if (onTimePickListener != null) {
				...
				onTimePickListener.onClick(selectHour, selectMinute);
			}
		} else if (v == btnCancel) {
...
void onClick(String hour, String minute);
	...
	public void setTime(int year, int month, int day, int hour, int minute) {
		...
		selectHour = hour + "";
		if (minute >= 0 && minute < 10) selectMinute = "0" + minute;
			else selectMinute = minute + "";
		issetdata = true;
...
//			initHours(hour);
//			initMinutes(minute);
			mHourAdapter.notifyDataChangedEvent();
			mMinuteAdapter.notifyDataChangedEvent();
			wvHour.invalidate();
			wvMinute.invalidate();
		}
