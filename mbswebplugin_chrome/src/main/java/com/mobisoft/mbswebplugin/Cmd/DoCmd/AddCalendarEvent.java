...
/**
 * Author：Created by fan.xd on 2019/1/28.
 * Email：fang.xd@mobisoft.com.cn
 * Description：添加日历
 */
public class AddCalendarEvent extends DoCmdMethod {
    ...
    @Override
    public String doMethod(HybridWebView webView, final Context context, com.mobisoft.mbswebplugin.MvpMbsWeb.Interface.MbsWebPluginContract.View view, com.mobisoft.mbswebplugin.MvpMbsWeb.Interface.MbsWebPluginContract.Presenter presenter, String cmd, final String params, String callBack) {
                ...
                public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
                    if (requestCode == 0x009) {
                        if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                            setCalend(context, params);
                        } else {
                            ToastUtil.showLongToast(context, "当前没有获得权限管理日历，无法添加日历事件");
                        }
                    }
                }
            });
    ...
    private void setCalend(Context context, String params) {
        try {
            ...
            String allDay = object.optString("allDay");
            long start = Long.valueOf(startDate);
            long end = Long.valueOf(endDate);
            int day = Integer.valueOf(allDay);

            addCalendarEvent(context, title, description, start, end, day);
    ...
    private void requestPermissions(Activity context) {
        if (ActivityCompat.shouldShowRequestPermissionRationale(context, Manifest.permission.WRITE_CALENDAR)) {
            ToastUtil.showLongToast(context, "需要此权限管理日历");
        } else {
            String[] per = {Manifest.permission.WRITE_CALENDAR};
            ActivityCompat.requestPermissions(context, per, 0x009);
        }
    }
     ...
     * 添加日历事件
     */
    public static void addCalendarEvent(Context context, String title, String description, long startMillis, long endMillis, int previousDate) {
        if (context == null) {
            return;
        }
...
//		long end = endTime;
        ...
        beginTime.set(2019, 1, 31, 12, 0, 0);
        long startMillis2 = beginTime.getTimeInMillis();
        Calendar endTime2 = Calendar.getInstance();
        endTime2.set(2019, 1, 31, 12, 0, 0);
        ...
        @SuppressLint("MissingPermission") Uri newEvent = cr.insert(CalendarContract.Events.CONTENT_URI, event);
        long eventID = Long.parseLong(newEvent.getLastPathSegment());
        ToastUtil.showLongToast(context, eventID + "ss");
