...
public class ViewUtil {
    ...
    public static int scaleValue(Context context, float value) {
        ...
        if (mDisplayMetrics.scaledDensity >= AppConfing.UI_DENSITY) {
            if (width > AppConfing.UI_WIDTH) {
                value = value * (1.3f - 1.0f / mDisplayMetrics.scaledDensity);
            } else if (width < AppConfing.UI_WIDTH) {
                value = value * (1.0f - 1.0f / mDisplayMetrics.scaledDensity);
            }
        } else {
            float offset = AppConfing.UI_DENSITY - mDisplayMetrics.scaledDensity;
            if (offset > 0.5f) {
                value = value * 0.9f;
            } else {
                value = value * 0.95f;
    ...
    public static int scale(int displayWidth, int displayHeight, float pxValue) {
        ...
        try {
            ...
            float scaleWidth = (float) width / AppConfing.UI_WIDTH;
            float scaleHeight = (float) height / AppConfing.UI_HEIGHT;
