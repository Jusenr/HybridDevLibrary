...
//先隐藏手势和控制栏，防止在没有prepare的时候做操作。
	...
	private void initControlView() {
		...
		mControlView.setOnSeekListener(new ControlView.OnSeekListener() {
			...
			public void onSeekEnd(int position) {
				...
				Log.i("oye", String.format("设置进度条的seek监听%s", position));
				if (isCompleted) {
					//播放完成了，不能seek了
					inSeek = false;
				} else {
			...
			public void onSeekStart() {
				//拖动开始
				inSeek = true;
				Log.i("oye", String.format("onSeekStart%s", getCurrentPosition()));
			}
		});
		//菜单按钮点击
	...
	private String getTitle(String title) {
		...
		if (mAliyunLocalSource != null) {
			...
			finalTitle = mAliyunVidSource.getTitle();
		}
	...
	private String getPostUrl(String postUrl) {
		...
		if (mAliyunLocalSource != null) {
			...
			finalPostUrl = mAliyunVidSource.getCoverUrl();
		}
	...
	public void setLocalSource(AliyunLocalSource aliyunLocalSource) {
		Log.i("oye","aliyunLocalSource 设置本地播放！");
		if (mAliyunVodPlayer == null) {
			return;
		}
	...
	public void setCoverUri(String uri) {
		...
		} else if (mCoverView != null && TextUtils.isEmpty(uri)) {
//			setCoverResource(R.drawable.alivc_playstate_play);
		}
	}
	 ...
	 * 开始进度条更新计时器
	 */
	private void startmSendProgress() {
		if (mSendProgress != null) {
			mSendProgress.removeMessages(0);
			mSendProgress.sendEmptyMessageDelayed(0, 5000);
		}
	}
...
//			Log.i("oye", " currentPosition = " + currentPosition + " , duration = " + mAliyunVodPlayer.getDuration());

			long currentPosition = mAliyunVodPlayer.getCurrentPosition();


			mControlView.setVideoPosition((int) currentPosition);
		}
	 ...
	 * 进度更新计时器
	 */
	private static class SendProgress extends Handler {

		private WeakReference<AliyunVodPlayerView> viewWeakReference;

		SendProgress(AliyunVodPlayerView aliyunVodPlayerView) {
			viewWeakReference = new WeakReference<AliyunVodPlayerView>(aliyunVodPlayerView);
		}
		...
		public void handleMessage(Message msg) {
			AliyunVodPlayerView aliyunVodPlayerView = viewWeakReference.get();
			if (aliyunVodPlayerView != null) {
				aliyunVodPlayerView.sendProgress();
			}
			super.handleMessage(msg);
		}
	}
	...
	private long sendProgress() {
		Intent tent = new Intent();// 广播的标签，一定要和需要接受的一致。
		JSONObject object = new JSONObject();
		long currentPosition = mAliyunVodPlayer.getCurrentPosition();

		try {
			long duration1 = mAliyunVodPlayer.getDuration();
			double l = (double) currentPosition / duration1;
			// 设置视观看时长
			int i = videoInfo.getPlayedTime() + 5;
			int time = videoInfo.getFrequency() + 5;


			object.put("frequency", time + "");


			int currentPosition2 = (int) (currentPosition / 1000);
			object.put("currentTime", currentPosition2);

			int duration2 = (int) (duration1 / 1000);
			object.put("totalTime", duration2 + "");
			object.put("currentTime", currentPosition2 + "");
			object.put("course_no", videoInfo.getCourse_no());
			object.put("courseItem_no", videoInfo.getCourseItem_no());
			object.put("videoPercentage", TimeFormater.formatFloat(l));

			object.put("type", "videoProgress’");


			if (TextUtils.equals(videoInfo.getStatus(), "play")) {
//				object.put("playedTime", i + "");
				object.put("playtime", 5 + "");
				videoInfo.setPlayedTime(i);
				object.put("status", videoInfo.getStatus());// play.pause

			} else if (TextUtils.equals(videoInfo.getStatus(), "close")) {
				object.put("playtime", "1");
				videoInfo.setPlayedTime(i);
				object.put("status", "play");// play.pause
			} else if (TextUtils.equals("switchPlayerState", videoInfo.getStatus())) {
				object.put("playtime", "1");
				videoInfo.setPlayedTime(i);
				object.put("status", "pause");// play.pause
			} else {
				object.put("playtime", "0");
				videoInfo.setPlayedTime(i);
				object.put("status", videoInfo.getStatus());// play.pause
			}
			String params = object.toString();
			tent.putExtra("status", videoInfo.getStatus());
			tent.putExtra("type", "videoProgress");
			tent.putExtra("data", params);
			tent.putExtra("function", "progressCall");
			tent.putExtra("frequency", time + "");
			tent.setAction("videovideoProgress");
//			tent.setAction("video");
			getmContext().sendBroadcast(tent);// 发
...
//        mPlayerState = mAliyunVodPlayer.getPlayerState();
	...
	public void onSart() {
		Log.i("oye","ali onSart 2150"+getClass().getName());
	...
	public void pause(String type) {
		...
		if (playerState == PlayerState.Started || mAliyunVodPlayer.isPlaying()) {
			if(playerState!=PlayerState.Paused){
				if (videoInfo != null && TextUtils.equals(type, "close")){
//					videoInfo.setStatus("close");
					sendPlayStatus("close");
				} else if (videoInfo != null && TextUtils.equals(type, "switchPlayerState")){
//					videoInfo.setStatus("switchPlayerState");
					sendPlayStatus("switchPlayerState");
				} else if (videoInfo != null){
					sendPlayStatus("pause");
				}
			}else {
				if (videoInfo != null){
					sendPlayStatus("pause");
	 ...
	 * @param status
	 */
	private void sendPlayStatus(String status) {
		videoInfo.setStatus(status);
		if (mSendProgress != null) {
			mSendProgress.removeMessages(0);
			mSendProgress.sendEmptyMessage(0);
		}
	}
	 ...
	 * @param strings
	 */
	public void setPlayInfo(String[] strings) {

		if (videoInfo == null) videoInfo = new VideoInfo();
		if (!TextUtils.isEmpty(strings[0]))
			videoInfo.setCourse_no(strings[0]);
		if (!TextUtils.isEmpty(strings[1]))
			videoInfo.setCourseItem_no(strings[1]);
		if (!TextUtils.isEmpty(strings[2]))
			videoInfo.setPlaceholderurl(strings[2]);
		if (!TextUtils.isEmpty(strings[3]))
			videoInfo.setPlayUrl(strings[3]);
		else
			videoInfo.setPlayUrl(null);
		if (!TextUtils.isEmpty(strings[4]))
			videoInfo.setLiveFlag(Boolean.valueOf(strings[4]));
		if (!TextUtils.isEmpty(strings[5]))
			videoInfo.setVideoPercentage(Integer.valueOf(strings[5]));
		videoInfo.setPlayedTime(0);
		videoInfo.setFrequency(0);
	}
	...
	public class VideoInfo implements Serializable {
		private String courseItem_no;// 视频编号
		...
		private String placeholderurl;// 图片地址

		private boolean liveFlag;// 是否直播

		private int videoPercentage;// 播放百分比

		private int playedTime;// 视频播放状态下的，已播放时长
		private int frequency;// 频率

		private String course_no;// 课件编号
...
//				return "http://www.weilonglatiao.cn/img/14065583367.jpg";
		...
		public void setVideoPercentage(int videoPercentage) {
